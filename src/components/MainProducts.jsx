  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-7 gap-x-40">
        ...
        {/* Products Column */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-1 md:gap-2 lg:gap-4 md:col-span-5  md:px-3 pt-6">
          {/* Display products */}
          {products.map((product) => {
            ...
            return (
              <Link key={id} to={`/products/${product.id}`}>
                <div className="px-2 md:px-4 lg:px-5 rounded-t-sm rounded-lg h-auto transition-transform duration-200 hover:md:shadow-lg hover:border hover:md:border-gray-300 hover:md:scale-105">
                  {/* Product Image */}
                  <img
                    src={image}
                    alt={name}
